...
def TestPre(path):
    ...
    df.replace((np.inf, -np.inf, np.nan), 0, inplace=True)
    # 替换标签值为 0 和 1
    df = df.replace({"Label": {'Benign': 0, 'DoS attacks-GoldenEye': 1, "DoS attacks-Slowloris": 1,
...
def TrainModel():
    ...
    # path3 = "./MachineLearningCVE/train.csv"
    ...
    # features = [f for f in data.columns if f not in ['Flow ID', 'Src IP', 'Dst IP', 'Src Port', 'Timestamp', 'Label',
    #                                                    "Dst Port", "Fwd Header Len", "Fwd Seg Size Min", "Fwd IAT Min", "Fwd Seg Size Min"
    #                                                  , "Pkt Len Max", "Init Fwd Win Byts", "Bwd Pkt Len Std"]]
    # features = [f for f in data.columns if f in ["Flow Packets/s", "Flow Byts/s",
...
def Test():
    # 构造测试数据
    test = []
    path = "./MachineLearningCVE/Thursday-15-02-2018_TrafficForML_CICFlowMeter.csv"
    df = pd.read_csv(path, skipinitialspace=True)
    # 处理缺失值及无穷
    df.replace((np.inf, -np.inf, np.nan), 0, inplace=True)
    # 替换标签值为 0 和 1
    data = df.replace({"Label": {'Benign': 0, 'DoS attacks-GoldenEye': 1, "DoS attacks-Slowloris": 1,
                                 'DDOS attack-HOIC': 1, 'DDOS attack-LOIC-UDP': 1, 'DoS attacks-SlowHTTPTest': 1,
                                 'DoS attacks-Hulk': 1}})
    features = [f for f in data.columns if f not in ['Flow ID', 'Src IP', 'Dst IP', 'Src Port', 'Timestamp', 'Label',
                                                     "Dst Port", "Fwd_Header_Len", "Fwd_Seg_Size_Min"]]
    # 分割
    x_train = data[features]
    print(lgb_pre(x_train[:100]))
    y_pred = lgb_pre_label(x_train)
...
def Pre(path, out):
    ...
    df.replace((np.inf, -np.inf, np.nan), 0, inplace=True)
    print("df", df)
    features = [f for f in df.columns if f not in ['Flow ID', 'Src IP', 'Dst IP', 'Src Port', 'Timestamp', 'Label',
                                                     'Protocol', 'Destination_Port', 'Destination Port', 'Dst Port']]
    # features = [f for f in df.columns if f in ["Dst Port", "Fwd Header Len", ""]]
    # features = [f for f in df.columns if f in ["Flow Packets/s", "Flow Byts/s",
    #                                            "Flow IAT Mean", "Flow IAT Std", "Flow IAT Max", "Flow IAT Min",
    #                                            "Fwd IAT Total", "Fwd IAT Tot",
    #                                                             "Fwd IAT Mean", "Fwd IAT Std", "Fwd IAT Max",
    #                                            "Fwd IAT Min",
...
if __name__ == '__main__':
    ...
    # TestPre("./MachineLearningCVE/Thurs day-15-02-2018_TrafficForML_CICFlowMeter.csv")
    # TestPre("./MachineLearningCVE/Friday-16-02-2018_TrafficForML_CICFlowMeter.csv")
    # Test()
    # Pre("./testdata/train.csv", "pre_testdata/ddos_pre_train.csv")
    # Pre("./MachineLearningCVE/Wednesday-21-02-2018_TrafficForML_CICFlowMeter.csv", "pre_testdata/ddos_pre_Wednesday.csv")
    # Pre("./testdata/normal1.pcap_Flow.csv", "pre_testdata/ddos_pre_normal1.csv")
    # Pre("./testdata/DDostest.pcap_Flow.csv", "pre_testdata/ddos_pre_ddostest.csv")
    # Pre("./testdata/123.pcap_Flow.csv", "pre_testdata/pre_123_pcap_Flow.csv")
    # Pre("./testdata/http.pcap_Flow.csv", "pre_testdata/pre_http_pcap_Flow.csv")
    # Pre("./testdata/test_mosaic.csv", "pre_testdata/pre_mosaic.csv")

    # Pre("./testdata/DDos_http.pcap_Flow.csv", "pre_testdata/pre_DDos_2.csv")
    # Pre("./testdata/DDos_http2.pcap_Flow.csv", "pre_testdata/pre_DDos_http2.csv")
    # Pre("./testdata/normal_flow.pcap_Flow.csv", "pre_testdata/pre_normal2.csv")
    Pre("./testdata/Friday-WorkingHours-Afternoon-DDos.pcap_ISCX.csv", "pre_testdata/Pre_Friday_WorkingHours.csv")
