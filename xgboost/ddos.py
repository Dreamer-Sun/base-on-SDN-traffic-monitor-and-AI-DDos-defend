    ...
    for i, (train_index, valid_index) in enumerate(kf.split(x_train, y_train)):
        ...
        if clf_name == "lgb":
            ...
            params = {
                ...
                'min_child_weight': 20,      # 叶节点样本的最少数量，默认值20，用于防止过拟合
...
def lgb_model(x_train, y_train):
    ...
    # 保存模型
    lgb_trained_model.save_model("model_ddos_lgb.txt")
...
def lgb_pre(X):
    # 模型加载
    lgb_trained_model = lgb.Booster(model_file='model_ddos_lgb.txt')
...
def train():
    ...
    features1 = [f for f in data.columns if f not in ['Flow ID', 'Src IP', 'Dst IP', 'Src Port', 'Timestamp', 'Dst Port', 'Label', "Protocol"]]
    # 分割
...
def test():
    ...
    # 处理缺失值及无穷
    df.replace((np.inf, -np.inf, np.nan), 0, inplace=True)
    features = [f for f in df.columns if f not in ['Flow ID', 'Src IP', 'Dst IP', 'Src Port', 'Timestamp', 'Dst Port', 'Label', "Protocol"]]
    # 分割
    x_train = df[features]
    ...
    df['pred'] = y_pred
    # 输出csv
    df.to_csv("ddos_pre.csv", index=False, sep=',')
